...
from .model import CLIP, CustomTextCLIP, CustomTextCLIP_visual ,convert_weights_to_lp, convert_to_custom_text_state_dict,\
...
def create_model_text(
    ...
    if custom_text:
        if "multimodal_cfg" in model_cfg:
            ...
            #logger.info("Creating CustomTextCLIP model")
    ...
    force_preprocess_cfg['size'] = 224
    set_model_preprocess_cfg(model, merge_preprocess_dict(preprocess_cfg, force_preprocess_cfg))

    return model

def create_model_visual(
        model_name: str,
        pretrained: Optional[str] = None,
        precision: str = 'fp32',
        device: Union[str, torch.device] = 'cpu',
    ...
    if custom_text:
        if "multimodal_cfg" in model_cfg:
            ...
            #logger.info("Creating CustomTextCLIP model")
            model = CustomTextCLIP_visual(**model_cfg, cast_dtype=cast_dtype)
            logger.info("Created CustomTextCLIP_visual model")
    else:
        model = CLIP(**model_cfg, cast_dtype=cast_dtype)
    ...
    if precision in ("fp16", "bf16"):
        ...
        logging.info("Creating model with fp32")
        model.to(device=device)
    ...
    if pretrained:
        ...
        if checkpoint_path:
            ...
            load_checkpoint(model, checkpoint_path, weights_only=load_weights_only,strict = False,device = device)
        else:
            error_str = (
                f'Pretrained weights ({pretrained}) not found for model {model_name}.'
                f' Available pretrained tags ({list_pretrained_tags_by_model(model_name)}.')
...
def create_model_and_transforms_text(
        model_name: str,
        pretrained: Optional[str] = None,
        precision: str = 'fp32',
        device: Union[str, torch.device] = 'cpu',
    ...
    #logger.info(" model = create_model")
    model = create_model_text(
        model_name,
        pretrained,
        precision=precision,
        device=device,
    ...
    return model, None,None #preprocess_train, preprocess_val


def create_model_and_transforms_visual(
        model_name: str,
        pretrained: Optional[str] = None,
        precision: str = 'fp32',
        device: Union[str, torch.device] = 'cpu',
    ...
    logger.info(" model = create_model_visual")
    model = create_model_visual(
        model_name,
        pretrained,
        precision=precision,
        device=device,
