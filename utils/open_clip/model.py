...
def _build_vision_tower(
    ...
    if vision_cfg.timm_model_name:
        logger.info(f"Using timm model {vision_cfg.timm_model_name} for vision tower")
        visual = TimmModel(
        ...
        logger.info(f"Using modified ResNet model for vision tower")
        ...
        logger.infpo(f"Using VisionTransformer model for vision tower")
...
class CustomTextCLIP_(nn.Module):
    output_dict: torch.jit.Final[bool]
...
class CustomTextCLIP(nn.Module):
    ...
    def encode_text(self, text, prompt=None,tokenized_prompts=None,normalize: bool = False):
        if prompt == None or tokenized_prompts == None:
            features = self.text(text)
        else:
...
class CustomTextCLIP_visual(nn.Module):
    output_dict: torch.jit.Final[bool]
