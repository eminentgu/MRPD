...
class TextTransformer(nn.Module):
    ...
    def _embeds(self, text) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        x_to_save = x.detach().cpu().numpy()
        np.savetxt('x.txt', x_to_save.reshape(-1, x_to_save.shape[-1]), fmt='%.6f') 
    ...
    def forward(self, text,ctx = None,tokenized_prompts=None):
        if ctx == None:
            x, attn_mask = self._embeds(text)
            ...
            n_ctx = ctx.shape[0] #3*1280
            n_cls = tokenized_prompts.shape[0] #40
            #logger.info(f"text shape : {text.shape}, ctx shape: {ctx.shape}, tokenized_prompts shape: {tokenized_prompts.shape}")
            if ctx.dim() == 2:
                ctx = ctx.unsqueeze(0).expand(n_cls, -1, -1) #40 3 1280
            with torch.no_grad():
                embedding = self.token_embedding(tokenized_prompts) #X X X A ...
            prefix = embedding[:, :1, :]
            ...
            x_to_save = prompts.detach().cpu().numpy()
            np.savetxt('prompts.txt', x_to_save.reshape(-1, x_to_save.shape[-1]), fmt='%.6f')  
            #logger.info(f"prompts shape : {prompts.shape}")
            cast_dtype = self.transformer.get_cast_dtype()
            seq_len = text.shape[1] #77
            attn_mask = self.attn_mask
            ...
            if self.cls_emb is not None:
                ...
                x = self.ln_final(x)
                pooled = text_global_pool(x, tokenized_prompts, pool_type=self.pool_type) #.argmax(dim=-1) #, 
                tokens = x
            if self.text_projection is not None:
                if isinstance(self.text_projection, nn.Linear):
                    pooled = self.text_projection(pooled)
