...
def fps6d(data, number):
    """
    FPS sampling on [B, N, 6] using only xyz (前3维), and gather full 6D (xyz+rgb) features.
    
    Args:
        data: torch.Tensor, shape [B, N, 6]
        number: int, number of points to sample

    Returns:
        fps_data: torch.Tensor, shape [B, number, 6]
    """
    data = data.float().unsqueeze(0) if data.dim() == 2 else data  # [1, N, 6] or [B, N, 6]
    xyz = data[:, :, :3].contiguous()  # [B, N, 3]

    # Use xyz for FPS
    fps_idx = pointnet2_utils.furthest_point_sample(xyz, number)  # [B, number]

    # Gather full 6D using FPS indices
    data_t = data.transpose(1, 2).contiguous()  # [B, 6, N]
    fps_data = pointnet2_utils.gather_operation(data_t, fps_idx)  # [B, 6, number]
    fps_data = fps_data.transpose(1, 2).contiguous()  # [B, number, 6]
    return fps_data
...
def knn_point(nsample, xyz, new_xyz):
    ...
    """
    #logger.info(xyz.device)
    #logger.info(new_xyz.device)
...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, args):
        ...
        self.patch_dropout = nn.Identity()#self.patch_dropout = PatchDropout(args.patch_dropout) if args.patch_dropout > 0. else nn.Identity()
        self.visual = point_transformer


    '''def forward(self, pts, colors, prompt = None):
        ...
        x = x + pos
        # x = x.half()
        logger.info("point cloud input shape is {}".format(x.shape))
    ...
    def forward(self, pts, colors, prompt=None):
        ...
        # pos embedding for patch tokens (based on center coords)
        pos = self.pos_embed(center)

        # handle prompt (if provided)
        if prompt is not None and prompt.shape[1] > 0:
            # prompt: [B, P, 768] assumed, check dimension match
            assert prompt.shape[0] == B and prompt.shape[2] == self.trans_dim, \
                f"Prompt shape must be [B, P, {self.trans_dim}], but got {prompt.shape}"

            # Optional: create zero pos encoding for prompt (or use learnable)
            prompt_pos = torch.zeros(B, prompt.shape[1], self.trans_dim, device=prompt.device)

            # concat: cls + prompt + patch
            #logger.info(cls_tokens.device)
            #logger.info(prompt.device)
            #logger.info(group_input_tokens.device)
            x = torch.cat((cls_tokens, prompt, group_input_tokens), dim=1)
            pos = torch.cat((cls_pos, prompt_pos, pos), dim=1)
        else:
            # no prompt
            x = torch.cat((cls_tokens, group_input_tokens), dim=1)
            pos = torch.cat((cls_pos, pos), dim=1)
        ...
        x = self.visual.norm(x[:, 0, :])  # take CLS token
        x = self.visual.fc_norm(x)
        x = self.trans2embed(x)
        return x